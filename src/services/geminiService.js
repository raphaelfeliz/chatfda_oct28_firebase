 ...
 */
const buildPrompt = (userInput, availableFacets, filteredProducts) => {
...
const isValidAIExtractionResponse = (obj) => {
    if (typeof obj !== 'object' || obj === null)
        return false;
    const o = obj;
    return (typeof o.knowledgeBaseAnswer === 'string' &&
 ...
 */
export const getAIResponseAndFacets = async (message, availableFacets, filteredProducts) => {
    // 1. Create the advanced prompt
    const engineeredPrompt = buildPrompt(message, availableFacets, filteredProducts);
    // 2. Define the fallback response in case of a network error
    const networkErrorResponse = {
        knowledgeBaseAnswer: 'Desculpe, não consegui me conectar ao assistente. Tente novamente mais tarde.',
        extractedFacets: {},
        talkToHuman: false,
        humanHandoffMessage: '',
    };
