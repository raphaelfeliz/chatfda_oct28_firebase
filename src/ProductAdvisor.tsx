...
// Corrected type/value imports & Removed unused Facet type
import type { Product, ChatMessage, CurrentQuestion } from './types.js';
...
======================================
*/
const ProductAdvisor: React.FC<AppProps> = ({ onClose }) => {
  ...
  */
  ...
  const [selectedFacets, setSelectedFacets] = useState<Record<string, string | null>>(initialFacets);
  // Holds facets extracted by AI but not yet confirmed/applied by the state machine
  const [tempExtractedFacets, setTempExtractedFacets] = useState<Record<string, string>>({});
  ...
  const [filteredProducts, setFilteredProducts] = useState<Product[]>(productData.productCatalog);
  ...
  */
  ...
  const calculateNextStep = useCallback((
    ...
    currentTempFacets: Record<string, string>, // Still expects strings here
  ) => {
    ...
    const applyFilters = (selections: Record<string, string | null>) => {
      return productData.productCatalog.filter(product => {
        return Object.entries(selections).every(([key, value]) => {
          if (value === null) return true; // Skip unselected facets
          // Special handling for width range
          if (key === 'largura') {
            const [min, max] = value.split('-').map(Number);
            if (isNaN(min) || isNaN(max)) return true; // Ignore invalid range format
        ...
        for (const facet of productData.facets) {
          ...
          if (newSelections[attribute] === null && value) {
            ...
            if (availableOptions.includes(value)) {
              ...
              const potentialMatches = availableOptions.filter(opt =>
    ...
    for (const facet of productData.facets) {
      ...
      if (newSelections[facet.attribute] === null) {
        ...
        if (facet.attribute === 'largura') {
          ...
          availableOptions = [...new Set(finalProducts.map(p => p[facet.attribute]?.toString()).filter(v => v !== null && v !== undefined))] as string[];
        }
        ...
        // If multiple options exist, this is the next question to ask the user
          ...
          return { // Return the state: this is the next question
    ...
    return { // Return the state: finished
  ...
  */
  ...
  const processStateTransition = useCallback((
    currentSelections: Record<string, string | null>,
    currentTemp: Record<string, string> // Input temp facets are strings
  ) => {
    // Call the MEMOIZED calculateNextStep function
    const {
      newSelections,
      newTempFacets, // Output temp facets are also strings
      filteredProducts: nextFilteredProducts,
      nextQuestion,
      isFinished: nextIsFinished
    } = calculateNextStep(currentSelections, currentTemp);
...
/*
  ...
  */
  ...
  const handleSelectOption = (attribute: string, value: string) => {
    ...
    if (nextQuestion) {
      ...
      const questionMessage: ChatMessage = { sender: MessageSender.BOT, text: questionText };
  ...
  const handleSendMessage = async (message: string) => {
    ...
    const {
      knowledgeBaseAnswer, // Can be string OR NULL
      humanHandoffMessage,
    ...
    const validFutureTempFacets: Record<string, string> = {}; // State machine expects only strings
    for (const key in combinedTempFacets) {
      if (Object.prototype.hasOwnProperty.call(combinedTempFacets, key) && combinedTempFacets[key] !== null) {
        validFutureTempFacets[key] = combinedTempFacets[key] as string;
      }
    }
    ...
    if (nextQuestion) {
        ...
        if (nextQuestion.facet.attribute === 'largura') {
            ...
            if (containsNumber && isLarguraQuery) questionText = "Escolha uma largura nas opções em roxo.";
        }
    ...
    if (messageQueue.length > 0) {
      ...
      const sendQueuedMessages = (index = 0) => {
        ...
        if (talkToHuman && messageText === humanHandoffMessage) setShowTalkToHuman(true);
        const botMessage: ChatMessage = { sender: MessageSender.BOT, text: messageText };
      ...
      const fallbackMessage: ChatMessage = { sender: MessageSender.BOT, text: "Não entendi bem. Poderia tentar descrever de outra forma ou escolher uma das opções?" };
  ...
  const handleBreadcrumbClick = (facetIndex: number) => {
  ...
  */
  ...
  const generateWhatsAppLink = useCallback(() => {
    ...
    const friendlyFacetTitles: Record<string, string> = {
  ...
  */
  ...
  return (
    <div className="w-full h-full bg-gray-800 text-white shadow-2xl flex flex-col">
      {/* ----- HEADER ----- */}
      <header className="flex justify-between items-center p-4 bg-[#14293D] border-b border-gray-700 flex-shrink-0">
        <div className="flex items-center">
          <img src="https://fabricadoaluminio.com.br/layout/favicon.ico" alt="Favicon da Fábrica do Alumínio" className="w-6 h-6 mr-3" />
          <h1 className="text-xl font-bold text-white">Especialista da Fábrica</h1>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="flex-grow flex flex-col md:flex-row min-h-0"> {/* min-h-0 prevents flexbox overflow */}

        {/* ----- LEFT PANEL: Breadcrumbs + Chat ----- */}
        <div className="md:w-1/2 flex flex-col border-b md:border-b-0 md:border-r border-gray-700">
          <Breadcrumbs facets={productData.facets} selectedFacets={selectedFacets} onBreadcrumbClick={handleBreadcrumbClick} />
          <div className="flex-grow min-h-0"> {/* Child needs min-h-0 for scrolling */}
            <ChatInterface
              messages={chatHistory}
              onSendMessage={handleSendMessage}
              isLoading={isLoadingAI}
              showTalkToHumanButton={showTalkToHuman}
              onTalkToHumanClick={handleTalkToHumanClick}
            />
          </div>
        </div>

        {/* ----- RIGHT PANEL: Dynamic (Question or Results) ----- */}
        <main className="md:w-1/2 flex flex-col">
          {showProductView ? (
            ...
            <FacetQuestion question={currentQuestion} onSelectOption={handleSelectOption} products={filteredProducts} />
          ) : (
            // Show loading state otherwise
            <div className="p-6 bg-gray-800 h-full flex items-center justify-center">
              <p className="text-gray-400">Carregando a próxima pergunta...</p>
            </div>
          )}
        </main>
      </div>
